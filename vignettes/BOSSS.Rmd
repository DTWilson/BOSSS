...
p <- here("vignettes", "example_files", "cRCT_prob.rds")
#saveRDS(prob, p)
prob <- readRDS(p)

p <- here("vignettes", "example_files", "cRCT_sol_init.rds")
#saveRDS(sol, p)
sol <- readRDS(p)
```
...
```{r, include = FALSE, eval=TRUE}
# Save problems and initial solution
p <- here("vignettes", "example_files", "cRCT_sol_final.rds")
#saveRDS(sol, p)
sol <- readRDS(p)
```
...
At any point, but especially after initialisation, we may decide that we need more points in our initial evaluations, or more simulations at each of the points evaluated so far. For example, suppose we started off with only 10 evaluations at each of 6 initial designs:
