                      ...
                      hypothesis = c(0.2, 15, 0.3, 15, 100, 10))
{
  ...
  fit <- glmer(y ~ trt + (1 | c_id), family = binomial, data = df)
...
bayes_det <- function(design = c(20, 10),
                      hypothesis = c(0.2, 15, 0.3, 15, 100, 10))
{
  return(c(0, design[1], 2*design[1]*design[2]))
}
```
...
hypotheses <- hypotheses(name = c("theta_0", "eff_n_0", "theta_1", "eff_n_1", "het_n_mean", "het_n_sd"),
                         param_matrix = matrix(c(0.2, 15, 0.3, 15, 100, 10), ncol = 1))
...
prob <- BOSSS_problem(sim_bayes, design_space, hypotheses, constraints, objectives, det_obj = bayes_det)
