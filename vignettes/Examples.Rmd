...
Note that we haven't specified a set of hypotheses; because we only need to simulate under a single hypothesis (to estimate the type II error rate), we can leave it to be specified implicitly through the default `hypothesis` argument of the simulation function.
...
```{r, include = FALSE, eval=TRUE}
# Save problems and initial solution
p <- here("vignettes", "example_files", "rec_prob.rds")
#saveRDS(prob, p)
prob <- readRDS(p)

p <- here("vignettes", "example_files", "rec_sol_init.rds")
#saveRDS(sol, p)
sol <- readRDS(p)
```
...
```{r, include = FALSE, eval=TRUE}
# Save problems and initial solution
p <- here("vignettes", "example_files", "rec_sol_final.rds")
#saveRDS(sol, p)
sol <- readRDS(p)
```
...
#ggsave(here("man", "figures", "rec.pdf"), height=9, width=14, units="cm")
```

If we want to revise a constraint, we can do that easily:

```{r}
prob <- update_constraint(prob, name = "TII_ass", nom = 0.5)
...
plot(sol)
#ggsave(here("man", "figures", "rec2.pdf"), height=9, width=14, units="cm")
```
